module sfincs_input
   !
   ! Parser for the SFINCS main input file `sfincs.inp`.
   !
   ! `sfincs.inp` is a flat keyword / value text file (one `key = value`
   ! pair per line, comment lines start with `#`, `!`, or `@`). Reads go
   ! through the generic `get_keyword(...)` interface provided by
   ! sfincs_read. Every call accepts an optional array of deprecated
   ! keyword aliases - the new key is tried first, then each legacy
   ! alias in order, then the caller's supplied default. A one-line
   ! warning is written to the log when an alias is matched, so users
   ! see that their .inp is using a deprecated name.
   !
   ! The module does not own the variables it fills - it writes directly
   ! into module-level state declared in sfincs_data.
   !
   ! Subroutines:
   !
   !   read_sfincs_input()
...
contains
   ...
   subroutine read_sfincs_input()
      ...
      !
      iok = check_file_exists('sfincs.inp', 'SFINCS input file', .true.)
      !
      ...
      call get_keyword(500, 'prcfile',                         prcpfile,                        'none',    [character(len=10) :: 'prcpfile', 'precipfile']) ! uniform precipitation time series
      ...
      if (bathtub) then
         ...
         ! input file paths to 'none' makes the discharge reader take its
         ! standard early-return path.
         !
         srcfile       = 'none'
         disfile       = 'none'
         netsrcdisfile = 'none'
