module sfincs_log
   ...
   !
   ! Core:
   !   - open_log / close_log / write_log : file handle + line writer
   !
   ! Formatted blocks (moved here from the former sfincs_screendump):
   !   - write_startup_log  : welcome banner + ASCII art + build info
   !   - write_processes_log: yes/no "Processes" summary
   !   - write_progress_log : per-timestep progress / ETA line
   !   - write_finished_log : end-of-run banner + timer summary +
   !                          average time step
   !
   integer        :: fid
